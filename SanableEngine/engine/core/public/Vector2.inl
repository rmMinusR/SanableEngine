...
///
/// Template class for two-element vectors, modified from a version for three element vectors.
...
public:
    ...
    const Vector2<TObj> operator < (const TObj s);            // compare each element with s, return vector of 1 or 0 based on test
    const Vector2<TObj> operator > (const TObj s);

    const Vector2<TObj> operator < (const Vector2<TObj>& v);  // element-wise less than comparion, return vector of 1 or 0 based on test
    const Vector2<TObj> operator > (const Vector2<TObj>& v);  // element-wise greater than comparion, return vector of 1 or 0 based on test

    bool operator == (const Vector2<TObj>& v);             // test vector for equality
    bool operator != (const Vector2<TObj>& v);             // test vector for inequality
...
template<class TObj>
inline const Vector2<TObj> Vector2<TObj>::operator < (const TObj s)
{
...
template<class TObj>
inline const Vector2<TObj> Vector2<TObj>::operator < (const Vector2<TObj>& v)
{
...
template <class TObj>
inline bool Vector2<TObj>::operator == (const Vector2<TObj>& v)
{
    return    _v[0] == v[0]
           && _v[1] == v[1];
}
...
template <class TObj>
inline const Vector2<TObj> operator + (const Vector2<TObj>& v, const TObj& s)
{
    return Vector2<TObj>(v) += s;
}
...
template <class TObj>
inline const TObj operator * (const Vector2<TObj>& v1, const Vector2<TObj>& v2)
{
    return v1.dot(v2);
}
...
template<typename T> const Vector2<T> Vector2_consts<T>::zero { 0, 0, 0 };
template<typename T> const Vector2<T> Vector2_consts<T>::one  { 1, 1, 1 };
template<typename T> const Vector2<T> Vector2_consts<T>::X { 1, 0, 0 };
template<typename T> const Vector2<T> Vector2_consts<T>::Y { 0, 1, 0 };
