...
void WidgetTransform::refresh() const
{
	...
	evaluatedVisibility = parent ? parent->evaluatedVisibility & ownVisibility : ownVisibility;
...
void WidgetTransform::setVisibility(WidgetVisibility visibility)
{
	ownVisibility = visibility;
	markDirty();
}

WidgetVisibility WidgetTransform::getVisibility() const
{
	if (dirty) refresh();
