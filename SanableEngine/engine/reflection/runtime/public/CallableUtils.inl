...
namespace stix::detail::CallableUtils
{
	...
	namespace Member
	{
		...
		using fully_erased_binder_t = void(*)(erased_fp_t fn, const SRef& returnValue, const SRef& thisObj, const std::vector<SRef>& parameters);
		...
		template<typename TReturn, typename TOwner, typename... TArgs>
		static void typeErasedInvoke(erased_fp_t erased_fn, const SRef& returnValue, const SRef& thisObj, const std::vector<SRef>& parameters)
		{
		...
		struct TypeEraser<TReturn, ReturnTypeGroup::Assignable>
		{
			...
			static void __impl(TReturn(TOwner::*fn)(TArgs...), const SRef& returnValue, const SRef& thisObj, const std::vector<SRef>& parameters, std::index_sequence<I...>)
			{
				//No need to check return type or owner type matching; this is handled in SAny::get
				
				//Check parameters match exactly
				bool good = TypeName::staticEqualsDynamic_many<std::vector<SRef>::const_iterator, true, TArgs...>(parameters.begin(), parameters.end());
				assert(good);
		...
		{
			template<typename TOwner, typename... TArgs, size_t... I>
			static void __impl(void(TOwner::*fn)(TArgs...), const SRef& returnValue, const SRef& thisObj, const std::vector<SRef>& parameters, std::index_sequence<I...>)
			{
				//No need to check return type or owner type matching; this is handled in SAny::get
	...
	namespace Static
	{
		...
		template<typename TReturn, typename... TArgs>
		static void typeErasedInvoke(TReturn(*fn)(TArgs...), const SRef& returnValue, const std::vector<SRef>& parameters)
		{
		...
		struct TypeEraser<TReturn, ReturnTypeGroup::Assignable>
		{
			...
			static void __impl(TReturn(*fn)(TArgs...), const SRef& returnValue, const std::vector<SRef>& parameters, std::index_sequence<I...>)
			{
				//No need to check return type or owner type matching; this is handled in SAny::get
			
				//Check parameters match exactly
				TypeName::staticEqualsDynamic_many<std::vector<SRef>::const_iterator, true, TArgs...>(parameters.begin(), parameters.end());
		...
		template<typename TReturn, typename... TArgs>
		using binder_t = void(*)(TReturn(*fn)(TArgs...), const SRef& returnValue, const std::vector<SRef>& parameters);
