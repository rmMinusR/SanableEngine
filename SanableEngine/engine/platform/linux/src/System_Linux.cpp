...
gpr460::System_Linux::System_Linux(GLSettings glSettings) :
...
gpr460::System_Linux::~System_Linux()
{
	assert(!isAlive);
	assert(windows.empty());
}

void gpr460::System_Linux::Init()
{
	System::Init();
...
void gpr460::System_Linux::DoMainLoop(void(*stepFn)(void*), void* stepArg)
{
	while (true)
	{
...
void gpr460::System_Linux::Shutdown()
{
	assert(isAlive);
	isAlive = false;
...
void gpr460::System_Linux::DebugPause()
{
#ifdef _DEBUG
...
void gpr460::System_Linux::ShowError(const std::wstring& message)
{
	MessageBoxW(NULL, message.c_str(), L"Error", MB_OK | MB_ICONSTOP);
}

void gpr460::System_Linux::LogToErrorFile(const std::wstring& message)
{
	//Lazy init logfile
	if (!logFile)
	{
...
std::vector<std::filesystem::path> gpr460::System_Linux::ListPlugins(std::filesystem::path path) const
{
...
std::filesystem::path gpr460::System_Linux::GetBaseDir() const
{
	//Get host assembly
...
void gpr460::System_Linux::pumpEvents()
{
	assert(isAlive);
	...
	{
		auto it = std::find_if(windows.begin(), windows.end(), [=](Window_Linux* w) { return w->sdlHandle == windowHandle; });
		if (it != windows.end()) return *it;
		else return (Window_Linux*)nullptr;
	};
	auto forwardToWindow = [&](SDL_Window* windowHandle)
	{
		Window_Linux* w = lookupWindow(windowHandle);
	...
	while (SDL_PollEvent(&event))
	{
		...
		{
			Window_Linux* window = lookupWindow(SDL_GetWindowFromID(event.window.windowID));
...
bool gpr460::System_Linux::isFocused(const Window* window)
{
	return window == currentFocus;
}

Window* gpr460::System_Linux::createWindow(const WindowSettings& settings, Application* engine)
{
	...
	SDL_SetWindowsMessageHook(
		...
		{
			static_cast<System_Linux*>(userdata)->handleNativeEvent((HWND)hWnd, message, wParam, lParam);
		},
		this
	);
	...
	Window_Linux* w = new Window_Linux(settings, engine, handle);
	windows.push_back(w);
	return w;
}

void gpr460::System_Linux::destroyWindow(Window* _window)
{
	Window_Linux* window = static_cast<Window_Linux*>(_window);
...
size_t gpr460::System_Linux::getNumWindows() const
{
	return windows.size();
}

Window* gpr460::System_Linux::getWindow(size_t which)
{
	return windows[which];
}

void gpr460::System_Linux::handleNativeEvent(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	for (Window_Linux* w : windows)
	{
