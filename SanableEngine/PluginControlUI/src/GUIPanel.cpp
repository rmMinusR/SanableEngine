...
GUI::Panel::Panel(const ControlID& theID, const Vector2D& relativePosition, const Vector2D& size, const Color& backgroundColor, GraphicsBufferManager* graphicsBufferManager) :
...
GUI::Panel::Panel(const ControlID& theID, const Vector2D& relativePosition, const Vector2D& size, const Color& backgroundColor, GraphicsBufferManager* graphicsBufferManager, std::initializer_list<Element*> children) :
...
void GUI::Panel::draw(GraphicsBuffer* dest)
{
	if (isActive())
	{
		...
		if (isDirty()) {
			...
			//Draw background
			GraphicsSystem::renderRect(*internalCanvas, ZERO_VECTOR2D, internalCanvas->getWidth(), internalCanvas->getHeight(), backgroundColor, true);
...
void GUI::Panel::rebuildAbsoluteTransform(const Vector2D& parentAbsPos)
{
	Element::rebuildAbsoluteTransform(parentAbsPos);
	rebuildChildAbsoluteTransforms();
}

void GUI::Panel::rebuildRenderTransform(const Vector2D& rootRenderPos)
{
	cachedRenderPosition = ZERO_VECTOR2D; //FIXME kludge
