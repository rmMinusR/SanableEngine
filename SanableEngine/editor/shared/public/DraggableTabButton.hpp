...
class DraggableTabView;
class ImageWidget;
class AnchoredPositioning;

class DraggableTabButton : public RadioButtonWidget
{
	std::function<bool(DraggableTabView*)> destinationFilter;
	Widget* content;
...
public:
	...
	EDITORSHARED_API void setDestinationFilter(std::function<bool(DraggableTabView*)> fn);
	EDITORSHARED_API void moveTo(DraggableTabView* destination, size_t index); // Ignores filter
...
private:
	DraggableTabView* findTabView(Vector2f pos) const;
};
